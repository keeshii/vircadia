...
void ScriptableAvatar::update(float deltatime) {
    ...
    if (_animation && _animation->isLoaded()) {
        ...
        if (_animation->isLoaded() && frames.size() > 0 && _geometryResource && _geometryResource->isHFMModelLoaded() /*&& !_bind.isNull() && _bind->isLoaded()*/) {
            if (!_isRigValid) {
